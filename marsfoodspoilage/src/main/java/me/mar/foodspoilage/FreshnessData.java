...
public record FreshnessData(long lastUpdateTick, long remainingFreshTicks, long initialFreshTicks) {
    ...
    public FreshnessData updatedTo(long currentTick) {
        ...
        return new FreshnessData(currentTick, remaining, this.initialFreshTicks);
    }

    public SpoilageState state(SpoilageProfile profile) {
        if (this.remainingFreshTicks <= 0) {
            return SpoilageState.SPOILED;
        }
        if (this.remainingFreshTicks <= profile.staleThresholdTicks()) {
